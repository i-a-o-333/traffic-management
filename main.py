...
class Dashboard:
    ...
    def _consume_live_poll_result(self):
        ...
        if got:
            ...
            if self.live.live_mode:
                self.ai.log("LIVE", "Fetched Open-Meteo/OSRM updates")
            else:
    ...
    def _update_route_summary(self, path_edges, route_cost):
        if path_edges:
            ...
            eta_google = self.live.external_eta_s / 60.0 if self.live.external_eta_s else None
            overlay = f" | OSRM ETA: {eta_google:.1f}m" if eta_google else ""
            self.route_text.set(f"Route edges: {len(path_edges)} | Dijkstra ETA: {eta_local:.1f}m{overlay}")
            self.travel_overlay_text.set(f"ETA Overlay: Dijkstra {eta_local:.1f}m" + (f" vs OSRM {eta_google:.1f}m" if eta_google else ""))
    ...
    def tick(self):
        ...
        for lat, lon in self.live.incident_points:
            ...
            if edge:
                self.ai.log("INCIDENT", f"Live map incident blocked edge {edge[0]}-{edge[1]}")

        if self.live.social_incidents:
            self.ai.log("SOCIAL", f"External incident signals: {len(self.live.social_incidents)} updates")
